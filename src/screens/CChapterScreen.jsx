...
import { getCQuestionsForChapter, C_MODE_LABELS } from '../data/c/questions';
import { getCQuestionState, isCQuestionUnlocked } from '../utils/cProgress';
import BackButton from '../components/BackButton';

export default function CChapterScreen({ chapterId, meta, onBack, onOpenQuestion, unlockNotice }) {
  const chapter = C_COURSE.chapters.find(item => item.id === chapterId);
  const c = meta?.codePaths?.c;
  const state = getCQuestionState(c, chapterId);
  ...
  return (
    <div style={styles.wrap} className="fade-in">
      ...
      {questions.length === 0 ? (
        <div style={styles.empty}>このチャプターの問題は準備中です。</div>
      ) : (
        <div style={styles.route}>
          {questions.map(question => {
            const unlocked = isCQuestionUnlocked(c, question);
            ...
            const label = C_MODE_LABELS[question.mode] || question.mode.toUpperCase();
            return (
              <button
                key={question.id}
                className={`sql-route-card ${current ? 'sql-route-card-current' : ''} ${question.mode === 'mission' ? 'sql-route-card-mission' : ''}`}
                style={{ ...styles.item, opacity: unlocked ? 1 : 0.45, borderColor: cleared ? 'var(--accent2)' : current ? '#ffa657' : 'rgba(255,166,87,0.25)' }}
                disabled={!unlocked}
...
const styles = {
  ...
  empty: { maxWidth: 1040, width: '100%', margin: '0 auto', color: 'var(--text-dim)', fontSize: 11, lineHeight: 2, padding: 16, border: '1px solid rgba(255,166,87,0.25)' },
  ...
  itemMode: { color: '#ffa657', fontSize: 8 },
  itemTitle: { color: 'var(--text)', fontSize: 10, lineHeight: 1.7 },
  itemStatus: { color: '#ffa657', fontSize: 8, marginTop: 'auto' },
};
