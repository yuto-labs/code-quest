...
import { getTypeScriptChapterStatus, getTypeScriptCourseCounts, getTypeScriptQuestionState } from '../utils/typescriptProgress';
import { SqlChapterProgress } from '../components/SqlComponents';
import BackButton from '../components/BackButton';

export default function TypeScriptPathScreen({ meta, onBack, onOpenChapter, onContinue, unlockNotice }) {
  const ts = meta?.codePaths?.typescript;
  const counts = getTypeScriptCourseCounts(ts);
  const hasResume = Boolean(ts?.resume?.questionId);
  return (
    <div style={styles.wrap} className="fade-in">
      <div style={styles.header}>
        <BackButton onClick={onBack} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={styles.title}>TypeScript PATH</div>
          <div style={styles.sub}>世界のテーマで型、安全な分岐、実用的な型モデルを学ぶ</div>
      ...
      {hasResume && (
        <button style={styles.continueBtn} onClick={onContinue}>
          ...
          <span style={styles.continueSub}>{ts.resume.chapterId} / {ts.resume.questionId}</span>
        </button>
      )}
      <div style={styles.chapterGrid}>
        {TYPESCRIPT_COURSE.chapters.map(chapter => {
          const status = getTypeScriptChapterStatus(ts, chapter.id);
          const state = getTypeScriptQuestionState(ts, chapter.id);
          ...
          return (
            <button
              ...
              style={{ ...styles.chapter, opacity: status === 'LOCKED' ? 0.45 : 1, borderColor: status === 'CLEARED' ? 'var(--accent2)' : '#3178c6' }}
...
const styles = {
  ...
  continueBtn: { width: '100%', maxWidth: 1040, margin: '0 auto', fontFamily: 'var(--pixel-font)', background: 'rgba(49,120,198,0.12)', border: '2px solid #7fb7ff', color: '#7fb7ff', padding: 14, textAlign: 'left', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 6 },
  ...
  chapter: { fontFamily: 'var(--pixel-font)', background: 'var(--panel)', border: '2px solid #3178c6', color: 'var(--text)', padding: 16, minHeight: 190, textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 10, cursor: 'pointer' },
  chapterNo: { color: 'var(--accent2)', fontSize: 10 },
  chapterTitle: { color: '#7fb7ff', fontSize: 12, lineHeight: 1.6 },
