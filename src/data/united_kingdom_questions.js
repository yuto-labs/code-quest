...
const jsExecute = [
  { id: 'gb_js_e01', languageId: 'javascript', world: 'execute', title: 'Alexander Fleming', titleJa: 'アレクサンダー・フレミング', category: 'science-history', objective: 'array', source: SRC.fleming, summaryJa: 'アレクサンダー・フレミングは1945年にノーベル生理学・医学賞を受賞しました。', prompt: 'map と join の出力を選んでください。', code: 'const names = ["fleming"];\nconst label = names.map(n => n.toUpperCase()).join("-");\nconsole.log(label);', options: ['FLEMING', 'fleming', 'F-L', 'Fleming'], answer: 'FLEMING', expectedOutput: 'FLEMING', hint: '各要素を大文字化します。', explanation: 'fleming が大文字になります。', programmingExplanation: 'map は各要素を変換し、join は配列を文字列にまとめます。', optionExplanations: { 'FLEMING': '正解。toUpperCase()でflemingがすべて大文字になり、要素が1つなのでjoinの区切り文字は使われずFLEMINGになります。', 'fleming': 'toUpperCase()が適用されるため、小文字のままのflemingが出力されることはありません。', 'F-L': 'namesの要素は"fleming"という1つの文字列全体で、文字単位に分割されて区切られるわけではありません。', 'Fleming': 'toUpperCase()は文字列全体を大文字化するため、先頭だけが大文字になることはありません。' } },
