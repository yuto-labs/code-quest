...
const pyExecute = [
  { id: 'de_py_e01', languageId: 'python', world: 'execute', title: 'Munich', titleJa: 'ミュンヘン', category: 'geography-city', objective: 'function', source: SRC.munich, summaryJa: 'ミュンヘンはバイエルン州の主要都市です。', prompt: '関数の戻り値を追跡し、出力を選んでください。', code: 'def city_label(name, state):\n    return f"{name}:{state}"\nprint(city_label("Munich", "Bavaria"))', options: ['Munich:Bavaria', 'Bavaria:Munich', 'Munich', 'Error'], answer: 'Munich:Bavaria', expectedOutput: 'Munich:Bavaria', hint: 'return の f-string で name と state の順番を見ます。', explanation: 'name が先、state が後ろなので Munich:Bavaria です。', programmingExplanation: '関数呼び出しでは引数が仮引数に渡され、return の式が出力されます。', optionExplanations: { 'Munich:Bavaria': '正解。f"{name}:{state}"はnameが先、stateが後なのでMunich:Bavariaになります。', 'Bavaria:Munich': 'f文字列は{name}:{state}の順で定義されているため、nameが先になります。', 'Munich': 'f文字列にはstateの値も含まれるため、nameだけが出力されることはありません。', 'Error': '渡された引数は文字列として正しく連結されるため、例外は発生しません。' } },
  { id: 'de_py_e02', languageId: 'python', world: 'execute', title: 'Brandenburg Gate', titleJa: 'ブランデンブルク門', category: 'landmark', objective: 'dictionary', source: SRC.brandenburgGate, summaryJa: 'ブランデンブルク門はベルリンにあります。', prompt: '辞書 lookup の結果を選んでください。', code: 'landmark = {"name": "Brandenburg Gate", "city": "Berlin"}\nprint(landmark["city"])', options: ['Berlin', 'Brandenburg Gate', 'Munich', 'city'], answer: 'Berlin', expectedOutput: 'Berlin', hint: '"city" キーの値を見ます。', explanation: 'landmark["city"] は Berlin を返します。', programmingExplanation: '辞書のキーを指定すると対応する値だけを取り出せます。', optionExplanations: { 'Berlin': '正解。landmark["city"]はcityキーの値であるBerlinを返します。', 'Brandenburg Gate': 'これはnameキーの値で、コードが読んでいるのはcityキーなので出力されません。', 'Munich': 'landmark辞書にはMunichという値は含まれておらず、cityの値はBerlinです。', 'city': '"city"はキーの名前そのものであり、辞書アクセスはキーに対応する値を返します。' } },
  { id: 'de_py_e03', languageId: 'python', world: 'execute', title: 'German rail route sample stops', titleJa: 'ドイツの鉄道路線サンプル停車地', category: 'transport', objective: 'loop', source: SRC.railRoute, summaryJa: 'サンプルとして Berlin、Leipzig などの鉄道停車地ラベルを扱います。', prompt: 'リストを順に連結した出力を選んでください。', code: 'stops = ["Berlin", "Leipzig"]\nresult = ""\nfor stop in stops:\n    result += stop + "|"\nprint(result[:-1])', options: ['Berlin|Leipzig', 'Leipzig|Berlin', 'Berlin|', 'Leipzig'], answer: 'Berlin|Leipzig', expectedOutput: 'Berlin|Leipzig', hint: 'stops の並び順と最後の | の削除を見ます。', explanation: 'Berlin、Leipzig の順に追加され、最後の区切りだけ削られます。', programmingExplanation: 'for ループで文字列を累積し、スライスで末尾1文字を除きます。', optionExplanations: { 'Berlin|Leipzig': '正解。stopsの順にBerlin|Leipzig|となり、[:-1]で末尾の|だけが取り除かれます。', 'Leipzig|Berlin': 'forループはstopsの並び順（Berlinが先）通りに処理するため、順序が入れ替わることはありません。', 'Berlin|': '[:-1]は末尾1文字だけを除くスライスで、2件目のLeipzigまで削ってしまうことはありません。', 'Leipzig': '1件目のBerlinも結果に含まれるため、2件目だけが残ることはありません。' } },
  { id: 'de_py_e04', languageId: 'python', world: 'execute', title: 'Black Forest region records', titleJa: 'シュヴァルツヴァルト（黒い森）の地域レコード', category: 'geography', objective: 'loop', source: SRC.blackForest, summaryJa: 'シュヴァルツヴァルトはドイツ南西部の地域です。', prompt: 'records を条件で絞った出力を選んでください。', code: 'records = [\n    {"name": "Black Forest", "southwest": True},\n    {"name": "Other", "southwest": False}\n]\nselected = [r["name"] for r in records if r["southwest"]]\nprint(selected[0])', options: ['Black Forest', 'Other', 'True', 'False'], answer: 'Black Forest', expectedOutput: 'Black Forest', hint: 'southwest が True の辞書だけ残します。', explanation: 'Black Forest の southwest だけが True なので選ばれます。', programmingExplanation: 'リスト内包表記は、条件に合う要素だけを新しいリストへ入れます。', optionExplanations: { 'Black Forest': '正解。southwestがTrueなのはBlack Forestだけなので、selectedにはBlack Forestのみが入りselected[0]はBlack Forestです。', 'Other': 'OtherのsouthwestはFalseなので、リスト内包表記の条件で除外されselectedには入りません。', 'True': 'selectedはname（文字列）のリストであり、southwestの真偽値そのものが出力されるわけではありません。', 'False': 'selected[0]はリストの先頭要素であるBlack Forest（文字列）であり、真偽値Falseが出力されることはありません。' } },
];

const pyFinalExecute = [
  { id: 'final_execute_DE_python_01', languageId: 'python', world: 'execute', title: 'Museum Island, Berlin records', titleJa: 'ベルリンの博物館島レコード', category: 'world-heritage', objective: 'loop', source: SRC.museumIsland, summaryJa: 'ベルリンの博物館島は世界遺産です。', prompt: '記録を絞り、年代の昇順に並べた出力を選んでください。', code: 'records = [\n    {"name": "Site A", "kind": "museum", "period": 200},\n    {"name": "Other", "kind": "park", "period": 100},\n    {"name": "Site B", "kind": "museum", "period": 50}\n]\nselected = sorted([r for r in records if r["kind"] == "museum"], key=lambda r: r["period"])\nprint(">".join(r["name"] for r in selected))', options: ['Site B>Site A', 'Site A>Site B', 'Other>Site A', 'Site A>Other'], answer: 'Site B>Site A', expectedOutput: 'Site B>Site A', hint: 'kind で絞ったあと period の昇順です。', explanation: 'museum は Site B(50) と Site A(200) で、昇順に Site B が先です。', programmingExplanation: 'フィルタリング、sorted の key、join を順番に追跡します。', optionExplanations: { 'Site B>Site A': '正解。kind==museumで絞るとSite A(200)とSite B(50)が残り、period昇順でSite B(50)が先になります。', 'Site A>Site B': 'periodの昇順で並べるとSite B(50)がSite A(200)より先になるため、この順序にはなりません。', 'Other>Site A': 'Otherのkindはparkでmuseumではないため、フィルタ条件で除外されます。', 'Site A>Other': 'Otherはmuseumではないため除外され、また並び順もperiod昇順とは異なります。' } },
  { id: 'final_execute_DE_python_02', languageId: 'python', world: 'execute', title: 'German cultural institutions dataset', titleJa: 'ドイツの文化機関データセット', category: 'culture', objective: 'dictionary', source: SRC.institutions, summaryJa: 'サンプルとして Berlin Philharmonic、Deutsches Museum、Goethe-Institut、Bauhaus Dessau を扱います。', prompt: '辞書とリストから berlin が True の件数を集計した結果を選んでください。', code: 'institutions = [\n    {"name": "Berlin Philharmonic", "berlin": True},\n    {"name": "Goethe-Institut", "berlin": True},\n    {"name": "Deutsches Museum", "berlin": False},\n    {"name": "Bauhaus Dessau", "berlin": False}\n]\ncount = sum(1 for i in institutions if i["berlin"])\nprint(count)', options: ['2', '4', '1', 'Berlin Philharmonic'], answer: '2', expectedOutput: '2', hint: 'berlin が True の辞書だけ数えます。', explanation: 'Berlin Philharmonic、Goethe-Institut の2件が True です。', programmingExplanation: '条件式を満たすたびに1を足し、sum で合計します。', optionExplanations: { '2': '正解。berlinがTrueなのはBerlin PhilharmonicとGoethe-Institutの2件なので、countは2になります。', '4': 'institutionsには4件のデータがありますが、berlinがTrueなのはそのうち2件だけです。', '1': 'Berlin PhilharmonicとGoethe-Institutの両方がTrueなので、1件だけがカウントされるわけではありません。', 'Berlin Philharmonic': 'countはTrueの件数を表す整数で、i自体の名前が出力されるわけではありません。' } },
];

const pyDebug = [
  { id: 'de_py_b01', languageId: 'python', world: 'debug', title: 'Euro formatting for Germany', titleJa: 'ドイツのユーロ表示', category: 'finance', objective: 'strings', source: SRC.euro, summaryJa: 'ユーロはドイツの通貨です。', prompt: '金額を 10.50 EUR と表示したいコードです。文字列と数値の扱いを直します。', code: 'amount = "10.5"\nprint(amount + 0 + " EUR")', cause: '文字列 amount に数値 0 を + で足している', fix: 'amount を float にして、f-string で小数2桁に整形する', impact: '数値計算と表示形式を分け、10.50 EUR と安定して表示できる', why: '文字列連結と数値加算は別の処理だからです。', correctedCode: 'amount = 10.5\nprint(f"{amount:.2f} EUR")', hint: '表示用の小数桁は f-string の書式で指定します。', explanation: '文字列と数値をそのまま足すと TypeError になります。', programmingExplanation: 'f"{value:.2f}" は小数2桁の文字列を作ります。', causeOptionExplanations: { '文字列 amount に数値 0 を + で足している': '正解。amountは文字列"10.5"で、文字列に数値0を+で足すことはできずTypeErrorになります。', '出力文の文字列だけが間違っている': '" EUR"の部分の書き方自体は正しく、問題はamountと0の加算部分にあります。', 'データの件数が不足している': 'amountには値が1つ入っており、件数ではなく型の不一致が原因です。' }, fixOptionExplanations: { 'amount を float にして、f-string で小数2桁に整形する': '正解。floatに変換しf"{amount:.2f}"とすれば、10.50 EURのように整形できます。', '条件を削除する': 'このコードにはif文などの条件はなく、削除すべき条件もありません。', 'すべての値を同じ文字列にする': '金額データを無視して固定文字列を出すと、実際の金額が反映されません。' }, impactOptionExplanations: { '数値計算と表示形式を分け、10.50 EUR と安定して表示できる': '正解。floatとf-stringの書式指定により、計算と表示形式を分けて安定して表示できます。', '常に例外を出して停止する': '修正後はamountがfloat型になるため、TypeError自体が発生しなくなります。', '題材データを使わなくなる': 'amountのデータはそのまま使われ続けます。修正は型と表示形式だけです。' } },
...
const jsExecute = [
  { id: 'de_js_e01', languageId: 'javascript', world: 'execute', title: 'Max Planck', titleJa: 'マックス・プランク', category: 'science-history', objective: 'array', source: SRC.maxPlanck, summaryJa: 'マックス・プランクは1918年にノーベル物理学賞を受賞しました。', prompt: 'map と join の出力を選んでください。', code: 'const names = ["planck"];\nconst label = names.map(n => n.toUpperCase()).join("-");\nconsole.log(label);', options: ['PLANCK', 'planck', 'P-L', 'Planck'], answer: 'PLANCK', expectedOutput: 'PLANCK', hint: '各要素を大文字化します。', explanation: 'planck が大文字になります。', programmingExplanation: 'map は各要素を変換し、join は配列を文字列にまとめます。', optionExplanations: { 'PLANCK': '正解。toUpperCase()でplanckがすべて大文字になり、要素が1つなのでjoinの区切り文字は使われずPLANCKになります。', 'planck': 'toUpperCase()が適用されるため、小文字のままのplanckが出力されることはありません。', 'P-L': 'namesの要素は"planck"という1つの文字列全体で、文字単位に分割されて区切られるわけではありません。', 'Planck': 'toUpperCase()は文字列全体を大文字化するため、先頭だけが大文字になることはありません。' } },
  ...
  { id: 'de_js_e03', languageId: 'javascript', world: 'execute', title: 'Deutsches Museum', titleJa: 'ドイツ博物館', category: 'museum', objective: 'object', source: SRC.deutschesMuseum, summaryJa: 'ドイツ博物館はミュンヘンにあります。', prompt: 'object と array の反復で作られる出力を選んでください。', code: 'const museum = { city: "Munich", collections: ["technology", "science"] };\nlet result = museum.city;\nfor (const item of museum.collections) {\n  result += ":" + item;\n}\nconsole.log(result);', options: ['Munich:technology:science', 'technology:science:Munich', 'Munich', 'science:technology'], answer: 'Munich:technology:science', expectedOutput: 'Munich:technology:science', hint: 'result は city から始まり、配列順に追加されます。', explanation: 'city の後ろに technology、science が順に追加されます。', programmingExplanation: 'オブジェクトの中の配列を for...of で順に処理します。', optionExplanations: { 'Munich:technology:science': '正解。resultはMunichから始まり、collectionsの順にtechnology、scienceが追加されます。', 'technology:science:Munich': 'resultはmuseum.cityで初期化されるため、Munichが先頭に来てcollectionsの要素はその後に追加されます。', 'Munich': 'for...ofループでcollectionsの2要素がresultに追加されるため、Munichだけで終わることはありません。', 'science:technology': 'collectionsの順序はtechnologyが先でscienceが後であり、また先頭のMunichも結果に含まれます。' } },
  { id: 'de_js_e04', languageId: 'javascript', world: 'execute', title: 'Frankfurt transport hub sample', titleJa: 'フランクフルトの交通拠点サンプル', category: 'transport', objective: 'function', source: SRC.frankfurt, summaryJa: 'フランクフルト・アム・マインはドイツの主要な金融・交通都市です。', prompt: '関数と条件の戻り値を選んでください。', code: 'function hubLabel(city) {\n  return city === "Frankfurt" ? "transport hub" : "other";\n}\nconsole.log(hubLabel("Frankfurt"));', options: ['transport hub', 'other', 'Frankfurt', 'true'], answer: 'transport hub', expectedOutput: 'transport hub', hint: '"Frankfurt" === "Frankfurt" の判定を見ます。', explanation: '条件が true なので "transport hub" が返ります。', programmingExplanation: '三項演算子は 条件 ? trueの値 : falseの値 で結果を選びます。', optionExplanations: { 'transport hub': '正解。city === "Frankfurt"がtrueになるため、三項演算子は"transport hub"を返します。', 'other': '条件が一致してtrueになるため、falseの場合の値である"other"は返りません。', 'Frankfurt': 'cityの値そのものではなく、三項演算子が返す"transport hub"という固定の文字列が表示されます。', 'true': '関数は文字列"transport hub"を返しており、条件式の真偽値そのものを返しているわけではありません。' } },
];

const jsFinalExecute = [
  { id: 'final_execute_DE_javascript_01', languageId: 'javascript', world: 'execute', title: 'Wadden Sea', titleJa: 'ワッデン海', category: 'world-heritage-nature', objective: 'array', source: SRC.waddenSea, summaryJa: 'ワッデン海はドイツなど複数国にまたがる世界遺産です。', prompt: 'objects を条件で絞り、名前をソートした出力を選んでください。', code: 'const sites = [\n  { name: "Site A", coastal: true, area: 5 },\n  { name: "Other", coastal: false, area: 3 },\n  { name: "Site B", coastal: true, area: 2 }\n];\nconst names = sites.filter(s => s.coastal).map(s => s.name).sort();\nconsole.log(names.join(">"));', options: ['Site A>Site B', 'Site B>Site A', 'Other>Site A', 'Other>Site B>Site A'], answer: 'Site A>Site B', expectedOutput: 'Site A>Site B', hint: 'coastal で絞ったあと、名前をアルファベット順にします。', explanation: 'Site A と Site B が残り、sort で Site A が先になります。', programmingExplanation: 'filter、map、sort、join を順に適用します。', optionExplanations: { 'Site A>Site B': '正解。coastalがtrueで絞るとSite AとSite Bが残り、sortのアルファベット順でSite Aが先になります。', 'Site B>Site A': 'sort()はアルファベット順に並べ替えるため、元の配列の出現順（Site Aが先）のままにはなりません。', 'Other>Site A': 'Otherのcoastalはfalseなため、filterの条件で除外されます。', 'Other>Site B>Site A': 'Otherはcoastalがfalseのためfilterで除外され、3件すべてが残ることはありません。' } },
  { id: 'final_execute_DE_javascript_02', languageId: 'javascript', world: 'execute', title: 'German rail and river-route sample data', titleJa: 'ドイツの鉄道・河川ルートサンプルデータ', category: 'transport', objective: 'object', source: SRC.routeData, summaryJa: 'サンプルとして Berlin–Munich 鉄道とライン川ルートのデータを扱います。', prompt: '入れ子配列を reduce で合計した結果を選んでください。', code: 'const routes = [{name:"Berlin-Munich", segments:[2,3]}, {name:"Rhine", segments:[5]}];\nconst total = routes.reduce((sum, route) => sum + route.segments.reduce((a,b) => a + b, 0), 0);\nconsole.log(total);', options: ['10', '5', '3', 'Berlin-Munich-Rhine'], answer: '10', expectedOutput: '10', hint: '内側の配列も合計します。', explanation: 'Berlin-Munich は5、Rhine は5、合計10です。', programmingExplanation: 'reduce を入れ子にすると、グループごとの合計と全体合計を順に作れます。', optionExplanations: { '10': '正解。Berlin-Munichのsegments(2+3=5)とRhineのsegments(5)を外側のreduceで合計すると5+5=10になります。', '5': 'これはBerlin-MunichまたはRhineどちらか1ルート分の内側のreduceの結果で、外側のreduceでの全体合計ではありません。', '3': 'これはBerlin-Munichのsegments配列の一部の値[3]にすぎず、内側・外側どちらのreduceの最終結果でもありません。', 'Berlin-Munich-Rhine': '+演算子は数値の加算として使われているため、ルート名が文字列として連結されることはありません。' } },
];

const jsDebug = [
  { id: 'de_js_b01', languageId: 'javascript', world: 'debug', title: 'German phone-number formatting', titleJa: 'ドイツの電話番号形式', category: 'administration', objective: 'strings', source: SRC.phone, summaryJa: 'サンプルとしてドイツの携帯電話番号は11桁で構成されるものとして扱います。', prompt: '11桁の番号を区切って表示したいコードです。長さと切り出しの不具合を直します。', code: 'const digits = "01512345678";\nif (digits.length === 10) {\n  console.log(`${digits.slice(0,4)} ${digits.slice(4)}`);\n}', cause: '11桁の番号なのに length === 10 と判定している', fix: 'length === 11 に修正してから slice する', impact: '11桁の番号を正しく整形できる', why: 'サンプル digits は11文字だからです。', correctedCode: 'const digits = "01512345678";\nif (digits.length === 11) {\n  console.log(`${digits.slice(0,4)} ${digits.slice(4)}`);\n}', hint: 'digits の文字数を数えます。', explanation: '条件が10のままだと整形処理に入りません。', programmingExplanation: '文字列 length と slice の開始・終了位置を合わせます。', causeOptionExplanations: { '11桁の番号なのに length === 10 と判定している': '正解。digitsは11文字の文字列ですが、条件がlength === 10なのでif文の中に入らず何も出力されません。', '出力文の文字列だけが間違っている': 'console.logとテンプレートリテラルの書き方自体は正しく、問題はif文の条件にあります。', 'データの件数が不足している': 'digitsは"01512345678"という11文字のデータを正しく持っており、件数不足ではありません。' }, fixOptionExplanations: { 'length === 11 に修正してから slice する': '正解。digitsの実際の長さである11に条件を合わせれば、if文の中に入りslice処理が実行されます。', '条件を削除する': '条件を削除すると桁数チェックが効かなくなり、想定外の長さの番号でも整形してしまいます。', 'すべての値を同じ文字列にする': '電話番号データを無視して固定文字列にすると、実際の番号が反映されません。' }, impactOptionExplanations: { '11桁の番号を正しく整形できる': '正解。条件をlength === 11に直すことで、11桁の番号が正しく区切って表示されるようになります。', '常に例外を出して停止する': 'この修正は条件式の値を直すだけで、例外を発生させるものではありません。', '題材データを使わなくなる': 'digitsの電話番号データはそのまま使われ続けます。修正は判定する桁数だけです。' } },
  { id: 'de_js_b02', languageId: 'javascript', world: 'debug', title: 'Berlin and Munich labels', titleJa: 'ベルリンとミュンヘン', category: 'geography-city', objective: 'object', source: SRC.twoCities, summaryJa: 'ベルリンとミュンヘンはドイツの主要都市です。', prompt: 'major が true の都市だけを表示したいコードです。プロパティ名の不一致を直します。', code: 'const cities = [{ name: "Berlin", major: true }, { name: "Munich", major: true }];\nconst labels = cities.filter(city => city.large === true).map(city => city.name);\nconsole.log(labels.join(","));', cause: 'データには major があるのに large を参照している', fix: 'city.major === true に修正する', impact: 'Berlin と Munich の両方を正しく選べる', why: '存在しない large は undefined になり、比較が true になりません。', correctedCode: 'const cities = [{ name: "Berlin", major: true }, { name: "Munich", major: true }];\nconst labels = cities.filter(city => city.major === true).map(city => city.name);\nconsole.log(labels.join(","));', hint: 'オブジェクトに実際にあるキーを確認します。', explanation: 'property name mismatch により条件が成立しません。', programmingExplanation: 'JavaScript では存在しないプロパティを読むと undefined になります。', causeOptionExplanations: { 'データには major があるのに large を参照している': '正解。各オブジェクトはmajorプロパティを持ちますが、フィルタ条件はcity.largeを見ているため常にundefinedとなり一致しません。', '出力文の文字列だけが間違っている': 'console.logとjoinの書き方自体は正しく、問題はfilterの条件にあります。', 'データの件数が不足している': 'citiesにはBerlinとMunichの2件があり、件数不足ではなくプロパティ名の不一致が原因です。' }, fixOptionExplanations: { 'city.major === true に修正する': '正解。実際に存在するmajorプロパティを参照することで、BerlinとMunichの両方が正しく一致します。', '条件を削除する': '条件を削除するとフィルタの意味がなくなり、絞り込みたい意図が反映されません。', 'すべての値を同じ文字列にする': '都市データを無視して固定文字列にすると、実際のmajorに基づく絞り込みができなくなります。' }, impactOptionExplanations: { 'Berlin と Munich の両方を正しく選べる': '正解。正しいプロパティ名majorで比較することで、両都市が正しく選ばれます。', '常に例外を出して停止する': '存在しないプロパティを読んでもJavaScriptではundefinedになるだけで、例外は発生しません。', '題材データを使わなくなる': 'BerlinとMunichのデータはそのまま使われ続けます。修正は参照するプロパティ名だけです。' } },
