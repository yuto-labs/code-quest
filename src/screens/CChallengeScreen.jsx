...
import { C_QUESTIONS_BY_ID, C_MODE_LABELS } from '../data/c/questions';
import { C_MAX_HEARTS } from '../utils/cProgress';
...
export default function CChallengeScreen({
  questionId,
  meta,
  onBack,
  onComplete,
  ...
  const progress = meta?.codePaths?.c;
  ...
  const [hearts, setHearts] = useState(saved.hearts ?? C_MAX_HEARTS);
  ...
  const chapterQuestions = useMemo(() => {
    if (!question) return [];
    return Object.values(C_QUESTIONS_BY_ID)
  ...
  const renderRoute = () => {
    ...
    return (
      <div className={`sql-step-route ${status === 'correct' ? 'sql-step-route-advance' : ''}`} aria-label={`C chapter progress ${question.order} of 10`}>
        {chapterQuestions.map(item => {
          ...
          return (
            <div
              ...
              title={`${item.order}. ${C_MODE_LABELS[item.mode] || item.mode}`}
            >
              {item.order}
            </div>
          );
  ...
  const renderExplanation = () => (
    <section style={styles.explanation}>
      ...
      <div style={styles.explainSection}>
        <div style={styles.sectionTitle}>C 解説</div>
      ...
      <div style={styles.wrap}>
        <div style={styles.resultShell}>
          <div ref={resultBodyRef} style={styles.resultBody}>
            ...
            <div style={styles.sub}>C PATH FAILED</div>
    ...
    return (
      <div style={styles.wrap} className="fade-in">
        <div style={styles.top}>
          <BackButton onClick={onBack} />
          <div style={styles.kicker}>{question.chapterId} / {C_MODE_LABELS[question.mode]} / Q{question.order}/10</div>
        </div>
        <main style={styles.resultBody}>
          {renderRoute()}
          <div style={styles.feedbackCorrect}>
            <div style={styles.feedbackTitle}>{isWrite ? 'CODE COMPLETED' : question.mode === 'fix' ? 'FIX APPLIED' : 'RESULT VERIFIED'}</div>
      ...
      <main style={styles.main}>
        <header style={styles.header}>
          <div style={styles.statusBar}>
            <div style={styles.kicker}>{question.chapterId} / {C_MODE_LABELS[question.mode]} / Q{question.order}/10</div>
            <div style={styles.hearts} aria-label={`hearts ${hearts} of ${C_MAX_HEARTS}`}>
              {Array.from({ length: C_MAX_HEARTS }).map((_, index) => (
...
const styles = {
  ...
  top: { flex: '0 0 auto', display: 'flex', gap: 10, justifyContent: 'space-between', padding: 'calc(env(safe-area-inset-top, 0px) + 14px) 14px 14px', borderBottom: '1px solid rgba(255,166,87,0.22)' },
  ...
  title: { margin: 0, color: '#ffa657', fontSize: 'clamp(18px, 5vw, 30px)', lineHeight: 1.5 },
  ...
  queryFill: { margin: 0, padding: '16px 18px', width: '100%', minWidth: 0, overflowX: 'auto', background: 'rgba(0,5,25,0.95)', border: '1px solid rgba(255,166,87,0.45)', color: '#ffe3c2', fontSize: 'clamp(13px, 3.5vw, 17px)', lineHeight: 1.9, whiteSpace: 'pre', fontFamily: 'var(--mono-font, ui-monospace, SFMono-Regular, Consolas, monospace)' },
  inlineBlank: { minWidth: 110, maxWidth: 'min(420px, 80vw)', background: 'rgba(255,166,87,0.14)', border: '2px solid #ffa657', color: '#ffe3c2', fontFamily: 'inherit', fontSize: 'clamp(13px, 3.5vw, 15px)', padding: '4px 7px', margin: '0 4px', outline: 'none' },
  ...
  activeFooter: { flex: '0 0 auto', padding: '10px 14px calc(env(safe-area-inset-bottom, 0px) + 10px)', background: 'rgba(13,13,26,0.96)', borderTop: '1px solid rgba(255,166,87,0.22)' },
  ...
  feedbackCorrect: { border: '2px solid rgba(255,166,87,0.5)', background: 'rgba(255,166,87,0.12)', color: '#ffa657', padding: 14, display: 'flex', flexDirection: 'column', gap: 10 },
  feedbackTitle: { fontFamily: 'var(--pixel-font)', fontSize: 12, textShadow: '0 0 10px rgba(255,166,87,0.45)' },
  answerLine: { display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', fontSize: 10, color: 'var(--text)' },
  explanation: { display: 'flex', flexDirection: 'column', gap: 14, border: '1px solid rgba(255,166,87,0.28)', background: 'rgba(255,166,87,0.07)', padding: 14 },
  explainSection: { display: 'flex', flexDirection: 'column', gap: 8 },
  sectionTitle: { color: '#ffa657', fontSize: 10, fontFamily: 'var(--pixel-font)' },
  ...
  footer: { flex: '0 0 auto', display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', padding: '12px 14px calc(env(safe-area-inset-bottom, 0px) + 12px)', background: 'rgba(13,13,26,0.96)', borderTop: '1px solid rgba(255,166,87,0.22)' },
