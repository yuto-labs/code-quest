...
function debugStep(num, title, prompt, question, options, answer, hint, explanation, optionExplanations) {
  return { stepNum: num, stepTitle: title, prompt, question, options, answer, hint, explanation, optionExplanations };
}
...
export const JP_JAVA_EXECUTE = [
  execute('jp_java_e01', 'strings', '有田焼: String.equals の追跡', '有田焼の産地ラベルを String.equals で判定したときの出力を選んでください。', 'public class Main {\n    public static void main(String[] args) {\n        String area = "有田";\n        if (area.equals("有田")) {\n            System.out.println("佐賀の磁器");\n        } else {\n            System.out.println("別の工芸");\n        }\n    }\n}', ['佐賀の磁器', '別の工芸', '有田', 'true'], '佐賀の磁器', 'String の内容比較は equals の結果を見ます。', 'area の中身は "有田" なので equals が true になり、最初の出力になります。', 'String.equals は文字列の内容が等しいかを判定します。', { '佐賀の磁器': '正解。area.equals("有田")はtrueになるため、if節の佐賀の磁器が出力されます。', '別の工芸': 'equalsによる文字列内容の比較はtrueになるため、else節の別の工芸は実行されません。', '有田': '出力されるのはif/elseで選ばれた固定文字列であり、area自体の値が出力されるわけではありません。', 'true': 'if文の条件式の評価結果ではなく、その分岐内で実行されるSystem.out.printlnの文字列が出力されます。' }),
  execute('jp_java_e02', 'arrays', '日本三名園: 配列を順に連結', '兼六園、後楽園、偕楽園の配列を順に処理した出力を選んでください。', 'public class Main {\n    public static void main(String[] args) {\n        String[] gardens = {"兼六園", "後楽園", "偕楽園"};\n        String result = "";\n        for (String garden : gardens) {\n            result += garden + " ";\n        }\n        System.out.println(result.trim());\n    }\n}', ['兼六園 後楽園 偕楽園', '偕楽園 後楽園 兼六園', '兼六園', 'Error'], '兼六園 後楽園 偕楽園', '拡張 for は配列の順番を保って進みます。', '配列の先頭から順に result へ足し、最後に trim で末尾の空白を消します。', '配列 iteration と文字列連結の順序を追います。', { '兼六園 後楽園 偕楽園': '正解。拡張forはgardensの順（兼六園が先）に処理し、trimで末尾の空白だけが取り除かれます。', '偕楽園 後楽園 兼六園': '拡張forは配列の出現順（兼六園が先）に処理するため、逆順になることはありません。', '兼六園': 'forループは3要素すべてをresultに連結するため、最初の1件だけで終わることはありません。', 'Error': 'String配列の拡張forとtrimの呼び出しはいずれも正常な操作で、例外は発生しません。' }),
  execute('jp_java_e03', 'methods', '1970年大阪万博: メソッド戻り値', '開催年を受け取るメソッドの戻り値を追ってください。', 'public class Main {\n    static String expoLabel(int year) {\n        return "大阪万博:" + year;\n    }\n    public static void main(String[] args) {\n        String label = expoLabel(1970);\n        System.out.println(label);\n    }\n}', ['大阪万博:1970', '1970:大阪万博', '大阪万博', 'Error'], '大阪万博:1970', 'return された文字列が label に入ります。', 'expoLabel(1970) は "大阪万博:" と 1970 を連結して返します。', 'メソッドの戻り値が変数へ代入され、出力に使われます。', { '大阪万博:1970': '正解。return "大阪万博:" + yearは"大阪万博:"が先、yearが後に連結されるためこの順になります。', '1970:大阪万博': 'return文の連結順は"大阪万博:"が先でyearが後であり、逆順にはなりません。', '大阪万博': '戻り値にはyearの値も連結されているため、文字列だけが出力されることはありません。', 'Error': '渡された引数は正しく文字列と連結されるため、例外は発生しません。' }),
  execute('jp_java_e04', 'collections', '釧路湿原: ArrayList filtering', '湿地として保護されている項目だけを ArrayList から抽出した出力を選んでください。', 'import java.util.ArrayList;\npublic class Main {\n    public static void main(String[] args) {\n        ArrayList<String> sites = new ArrayList<>();\n        sites.add("釧路湿原:湿地");\n        sites.add("摩周湖:湖");\n        for (String site : sites) {\n            if (site.contains("湿地")) {\n                System.out.println(site.split(":")[0]);\n            }\n        }\n    }\n}', ['釧路湿原', '摩周湖', '湿地', '釧路湿原\n摩周湖'], '釧路湿原', 'contains("湿地") を満たす要素だけが出力されます。', '1件目だけが条件に合い、split(":")[0] で名前部分だけを取り出します。', 'ArrayList の各要素を条件で絞り、文字列を分割します。', { '釧路湿原': '正解。"釧路湿原:湿地"だけがcontains("湿地")を満たし、split(":")[0]で名前部分の釧路湿原が出力されます。', '摩周湖': '"摩周湖:湖"にはcontains("湿地")の条件を満たす文字列が含まれず、if文の中に入りません。', '湿地': 'split(":")[0]は":"より前の部分を取り出すため、出力されるのは釧路湿原で湿地ではありません。', '釧路湿原\n摩周湖': 'if (site.contains("湿地"))で絞られるため、条件を満たさない摩周湖は出力されません。' }),
  execute('jp_java_e05', 'collections', '日本の伝統色: HashMap lookup', 'HashMap に入れた色名データから、紅の説明を取り出す出力を選んでください。', 'import java.util.HashMap;\npublic class Main {\n    public static void main(String[] args) {\n        HashMap<String, String> colors = new HashMap<>();\n        colors.put("藍", "blue");\n        colors.put("紅", "red");\n        colors.put("若草", "green");\n        System.out.println(colors.get("紅"));\n    }\n}', ['red', 'blue', 'green', '紅'], 'red', 'HashMap はキーに対応する値を返します。', 'キー "紅" に対応する値は "red" です。', 'put と get のキー対応を追います。', { 'red': '正解。colors.get("紅")はキー"紅"に対応する値"red"を返します。', 'blue': 'これはキー"藍"に対応する値で、コードが読んでいるのは"紅"キーなので出力されません。', 'green': 'これはキー"若草"に対応する値で、"紅"の値ではありません。', '紅': '"紅"はキーの名前そのものであり、getはキーに対応する値を返します。' }),
  execute('jp_java_e06', 'classes', '文楽: コンストラクタとフィールド', '文楽データのオブジェクトを作った後、出力される値を選んでください。', 'class Performance {\n    String name;\n    String type;\n    Performance(String name, String type) {\n        this.name = name;\n        this.type = type;\n    }\n}\npublic class Main {\n    public static void main(String[] args) {\n        Performance bunraku = new Performance("文楽", "人形劇");\n        System.out.println(bunraku.name + ":" + bunraku.type);\n    }\n}', ['文楽:人形劇', '人形劇:文楽', '文楽', 'Error'], '文楽:人形劇', 'コンストラクタで name と type に入る順番を追います。', 'new Performance("文楽", "人形劇") により name は文楽、type は人形劇です。', 'コンストラクタ引数がフィールドに代入される流れを追います。', { '文楽:人形劇': '正解。コンストラクタの第1引数"文楽"がname、第2引数"人形劇"がtypeに入り、name + ":" + typeの順で連結されます。', '人形劇:文楽': '出力はbunraku.name + ":" + bunraku.typeの順であり、nameが先、typeが後になります。', '文楽': 'bunraku.typeも結果に含まれるため、nameだけが出力されることはありません。', 'Error': 'コンストラクタ呼び出しと文字列連結はいずれも正常な操作で、例外は発生しません。' }),
  execute('jp_java_e07', 'inheritance', '空手と琉球: オーバーライド', '継承したクラスのメソッド呼び出しで、どの出力になるか選んでください。', 'class MartialArt {\n    String origin() { return "武道"; }\n}\nclass Karate extends MartialArt {\n    @Override\n    String origin() { return "琉球・沖縄"; }\n}\npublic class Main {\n    public static void main(String[] args) {\n        MartialArt art = new Karate();\n        System.out.println(art.origin());\n    }\n}', ['琉球・沖縄', '武道', 'Karate', 'Error'], '琉球・沖縄', '変数の型ではなく、実際のオブジェクトが Karate である点を見ます。', '実体は Karate なので、オーバーライドされた origin() が呼ばれます。', 'Java の動的ディスパッチでは、実際のオブジェクト側のメソッドが選ばれます。', { '琉球・沖縄': '正解。artの実体（実際に生成されたオブジェクト）はKarateなので、overrideされたorigin()が呼ばれます。', '武道': '変数の宣言型はMartialArtですが、実行時にはKarateのoverrideされたメソッドが呼ばれるため親クラスの値は使われません。', 'Karate': 'これは型の名前であり、origin()メソッドが返す文字列ではありません。', 'Error': 'KarateはMartialArtを正しく継承してoverrideしているため、コンパイルも実行も問題なく行われます。' }),
];
...
export const JP_JAVA_DEBUG = [
  debug('jp_java_b01', 'strings', '漆・漆器: == と equals のバグ', '漆器の材料名を判定する Java コードのバグを、原因・修正・影響の3段階で確認します。', 'public class Main {\n    public static void main(String[] args) {\n        String material = new String("漆");\n        if (material == "漆") {\n            System.out.println("漆器の塗料");\n        }\n    }\n}', [
    debugStep(1, '原因を特定', '文字列の内容は同じなのに出力されません。', '主な原因はどれですか。', ['String を == で比較している', 'println がない', '変数 material が int 型'], 'String を == で比較している', 'String の内容比較と参照比較の違いを見ます。', '== は同じオブジェクト参照かを見ます。', { 'String を == で比較している': '正解。new String("漆")で作られたmaterialはリテラル"漆"とは別オブジェクトなので、==による参照比較はfalseになります。', 'println がない': 'System.out.printlnはif文の中に正しく書かれており、欠けているわけではありません。', '変数 material が int 型': 'materialはString型として宣言されており、int型ではありません。' }),
    debugStep(2, '修正を選ぶ', '内容が同じかを判定する最小修正を選びます。', '正しい修正はどれですか。', ['material.equals("漆") を使う', 'material = "漆" を使う', 'if を消す'], 'material.equals("漆") を使う', 'String の内容比較メソッドを使います。', 'equals で文字列の内容を比較できます。', { 'material.equals("漆") を使う': '正解。equalsは文字列の内容を比較するため、別オブジェクトでも内容が同じなら一致判定できます。', 'material = "漆" を使う': '代入は比較ではなく、if文の条件として使うとコンパイルエラーになります。', 'if を消す': '条件を削除すると常に漆器の塗料と表示されてしまい、判定の意味がなくなります。' }),
    debugStep(3, '影響を説明', '修正後の動作を確認します。', '修正の影響として正しいものはどれですか。', ['同じ内容の文字列なら漆器の塗料と出力できる', 'すべての文字列で必ず出力される', 'null でも必ず安全になる'], '同じ内容の文字列なら漆器の塗料と出力できる', '条件が true になるケースを見ます。', '内容が漆なら出力されます。', { '同じ内容の文字列なら漆器の塗料と出力できる': '正解。equalsにより、内容が"漆"と一致する場合に正しく漆器の塗料が出力されます。', 'すべての文字列で必ず出力される': 'equalsは内容が一致した場合のみtrueになるため、すべての文字列で出力されるわけではありません。', 'null でも必ず安全になる': 'materialがnullの場合、material.equals(...)を呼ぶとNullPointerExceptionになるため安全とは言えません。' }),
  ...
  debug('jp_java_b02', 'arrays', '七福神: 配列添字の範囲外', '七福神の配列を扱うコードの範囲外アクセスを修正します。', 'public class Main {\n    public static void main(String[] args) {\n        String[] gods = {"恵比寿", "大黒天", "毘沙門天", "弁財天", "福禄寿", "寿老人", "布袋"};\n        System.out.println(gods[7]);\n    }\n}', [
    debugStep(1, '原因を特定', '7柱の名前を持つ配列で例外が起きます。', '原因はどれですか。', ['添字7は8番目を指すため範囲外', '配列が空', 'String が使えない'], '添字7は8番目を指すため範囲外', 'Java の配列添字は0から始まります。', '7要素の最後は添字6です。', { '添字7は8番目を指すため範囲外': '正解。godsは7要素（添字0〜6）しかなく、添字7は8番目を指すためArrayIndexOutOfBoundsExceptionになります。', '配列が空': 'godsには7柱の名前が正しく格納されており、空ではありません。', 'String が使えない': 'String配列の宣言・初期化自体は正しく書けており、Stringが使えないわけではありません。' }),
    debugStep(2, '修正を選ぶ', '最後の要素を出したい場合の修正はどれですか。', '正しい修正はどれですか。', ['gods[6] にする', 'gods[0] にする', 'gods[8] にする'], 'gods[6] にする', '7番目の要素は添字6です。', '最後の要素へ正しくアクセスします。', { 'gods[6] にする': '正解。7要素の配列の最後の有効な添字は6なので、gods[6]で布袋を安全に取得できます。', 'gods[0] にする': 'これは最初の要素（恵比寿）を指し、最後の要素を取得する目的には合いません。', 'gods[8] にする': 'gods[8]は添字7よりさらに範囲外で、同じ例外がより確実に発生します。' }),
    debugStep(3, '影響を説明', '修正後の出力を確認します。', '修正の影響として正しいものはどれですか。', ['ArrayIndexOutOfBoundsException を避けられる', '配列の要素数が8になる', 'すべての神名が出る'], 'ArrayIndexOutOfBoundsException を避けられる', '例外原因が消えるかを見ます。', '範囲内の添字になり、例外を避けられます。', { 'ArrayIndexOutOfBoundsException を避けられる': '正解。gods[6]は配列の範囲内なので、例外を起こさず布袋を出力できます。', '配列の要素数が8になる': '修正はアクセスする添字を変えるだけで、配列の要素数（7個）自体は変わりません。', 'すべての神名が出る': 'System.out.printlnは1つのgods[6]だけを出力するコードのため、全員分は表示されません。' }),
  ...
  debug('jp_java_b03', 'null-safety', '江戸切子: null にメソッド呼び出し', '江戸切子の地域名データが null のときに落ちるコードを修正します。', 'public class Main {\n    public static void main(String[] args) {\n        String craftArea = null;\n        if (craftArea.equals("東京")) {\n            System.out.println("江戸切子");\n        }\n    }\n}', [
    debugStep(1, '原因を特定', '実行時に NullPointerException が起きます。', '原因はどれですか。', ['null に対して equals を呼んでいる', '東京が文字列ではない', 'println が多すぎる'], 'null に対して equals を呼んでいる', 'メソッドを呼んでいる左側の値を見ます。', 'craftArea が null なので equals を呼べません。', { 'null に対して equals を呼んでいる': '正解。craftAreaはnullなので、craftArea.equals("東京")を呼ぶとNullPointerExceptionになります。', '東京が文字列ではない': '"東京"は正しい文字列リテラルであり、型の問題ではありません。', 'println が多すぎる': 'System.out.printlnはこのコードに1回しか出てこず、数の問題ではありません。' }),
    debugStep(2, '修正を選ぶ', '安全に比較する修正はどれですか。', '正しい修正はどれですか。', ['craftArea != null && craftArea.equals("東京")', 'craftArea == null && craftArea.equals("東京")', 'craftArea.equals(null)'], 'craftArea != null && craftArea.equals("東京")', 'null でないことを先に確認します。', '&& は左が false なら右を評価しません。', { 'craftArea != null && craftArea.equals("東京")': '正解。&&は左がfalseのとき右を評価しないため、craftAreaがnullならequalsを呼ばずに安全にfalseとなります。', 'craftArea == null && craftArea.equals("東京")': 'craftAreaがnullのとき左がtrueになり右のequalsが評価され、結局NullPointerExceptionが起きます。', 'craftArea.equals(null)': 'これはcraftArea自体がnullなら依然として例外になり、また比較対象もnullになってしまいます。' }),
    debugStep(3, '影響を説明', '修正後の動作を確認します。', '修正の影響として正しいものはどれですか。', ['null のとき例外を避けられる', 'null のとき必ず江戸切子と出る', 'equals が不要になる'], 'null のとき例外を避けられる', 'null の場合に右側が呼ばれない点を見ます。', '例外を避けつつ、東京のときだけ出力できます。', { 'null のとき例外を避けられる': '正解。&&の短絡評価により、craftAreaがnullのときequalsが呼ばれず例外を避けられます。', 'null のとき必ず江戸切子と出る': 'craftAreaがnullのときは条件全体がfalseになり、江戸切子は出力されません。', 'equals が不要になる': '修正後もequalsは"東京"との内容比較に使われており、不要にはなりません。' }),
  ...
  debug('jp_java_b04', 'static', '東京タワー: static と instance', '東京タワーの開業年を扱うコードで、static ではないフィールドをクラス名から読んでいるバグを修正します。', 'class Tower {\n    int openedYear = 1958;\n}\npublic class Main {\n    public static void main(String[] args) {\n        System.out.println(Tower.openedYear);\n    }\n}', [
    debugStep(1, '原因を特定', 'コンパイルエラーになります。', '原因はどれですか。', ['インスタンスフィールドをクラス名から読んでいる', '1958 が int ではない', 'class が使えない'], 'インスタンスフィールドをクラス名から読んでいる', 'openedYear に static があるか見ます。', 'openedYear はオブジェクトに属するフィールドです。', { 'インスタンスフィールドをクラス名から読んでいる': '正解。openedYearはstaticではない通常のフィールドなので、Tower.openedYearのようにクラス名から直接読むことはできません。', '1958 が int ではない': '1958は通常の整数リテラルで、int型として正しく扱えます。', 'class が使えない': 'class Towerの定義自体は正しいJava構文で、問題はフィールドへのアクセス方法です。' }),
    debugStep(2, '修正を選ぶ', '最小修正として適切なのはどれですか。', '正しい修正はどれですか。', ['Tower tower = new Tower(); System.out.println(tower.openedYear);', 'System.out.println(openedYear);', 'Tower.openedYear() にする'], 'Tower tower = new Tower(); System.out.println(tower.openedYear);', 'オブジェクトを作ってから読みます。', 'new Tower() でインスタンスを作り、tower.openedYear を読みます。', { 'Tower tower = new Tower(); System.out.println(tower.openedYear);': '正解。インスタンスを生成し、そのオブジェクトを通じてopenedYearを読むことで正しくアクセスできます。', 'System.out.println(openedYear);': 'main内にopenedYearという変数は存在しないため、コンパイルエラーになります。', 'Tower.openedYear() にする': 'openedYearはメソッドではなくフィールドなので、()を付けて呼び出すことはできません。' }),
    debugStep(3, '影響を説明', '修正後の出力を確認します。', '修正の影響として正しいものはどれですか。', ['1958 を出力できる', 'static フィールドに変わる', 'クラス定義が不要になる'], '1958 を出力できる', 'どの所有者から読むかを見ます。', 'インスタンスから読むことで開業年を表示できます。', { '1958 を出力できる': '正解。インスタンスtowerを生成しtower.openedYearを読むことで、初期値1958が正しく出力されます。', 'static フィールドに変わる': '修正はインスタンスから読む方法に変えるだけで、openedYearの宣言自体は変わりません。', 'クラス定義が不要になる': 'Towerクラスはインスタンスを生成するために必要であり、修正後も定義は必要です。' }),
  ...
  debug('jp_java_b05', 'loops', '十二支: off-by-one ループ', '十二支の配列をすべて出すループの終了条件を修正します。', 'public class Main {\n    public static void main(String[] args) {\n        String[] zodiac = {"子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"};\n        for (int i = 0; i <= zodiac.length; i++) {\n            System.out.println(zodiac[i]);\n        }\n    }\n}', [
    debugStep(1, '原因を特定', '最後で配列範囲外になります。', '原因はどれですか。', ['i <= zodiac.length で length 番目まで進む', '配列が11個しかない', 'i++ がない'], 'i <= zodiac.length で length 番目まで進む', '配列の最後の添字は length - 1 です。', 'i が 12 になったとき範囲外です。', { 'i <= zodiac.length で length 番目まで進む': '正解。zodiac.lengthは12ですが<=だとiが12のときも実行され、zodiac[12]は範囲外になります。', '配列が11個しかない': 'zodiacには子から亥まで正しく12個の要素が格納されています。', 'i++ がない': 'forループにはi++が正しく書かれており、増分処理は欠けていません。' }),
    debugStep(2, '修正を選ぶ', '12個を安全に出す条件はどれですか。', '正しい修正はどれですか。', ['i < zodiac.length', 'i <= zodiac.length - 2', 'i == zodiac.length'], 'i < zodiac.length', '0 から length - 1 まで進めます。', '< を使うと最後の有効添字で止まります。', { 'i < zodiac.length': '正解。i < zodiac.lengthはiが0から11まで進み、12個すべての要素を範囲内で処理できます。', 'i <= zodiac.length - 2': 'この条件だとiは0から10までしか進まず、最後の亥が出力されません。', 'i == zodiac.length': '==だとiが0からlengthまで増える間ループ自体が一度も実行されず、何も出力されません。' }),
    debugStep(3, '影響を説明', '修正後の動作を確認します。', '修正の影響として正しいものはどれですか。', ['12個すべてを範囲内で出力できる', '最後の亥だけ出ない', '13個出る'], '12個すべてを範囲内で出力できる', '0から11までの12回を確認します。', '十二支の12要素をすべて安全に出力できます。', { '12個すべてを範囲内で出力できる': '正解。i < zodiac.lengthによりiは0から11までの12回ループし、12要素すべてを範囲内で出力できます。', '最後の亥だけ出ない': '修正後は添字11（亥）まで正しくループするため、亥も出力されます。', '13個出る': 'zodiacは12要素しかなく、ループも12回しか実行されないため13個出ることはありません。' }),
...
export const JP_JAVA_FINAL_MISSIONS = {
  ...
  execute: {
    ...
    unlock: { requiresStageClear: true },
    questions: [
      execute('final_execute_JP_java_01', 'collections', '琉球王国のグスク: final filtering/order', '登録年2000のグスク関連データだけを順に出力した結果を選んでください。', 'import java.util.ArrayList;\nclass Site {\n    String name; int year; int order;\n    Site(String name, int year, int order) { this.name = name; this.year = year; this.order = order; }\n}\npublic class Main {\n    public static void main(String[] args) {\n        ArrayList<Site> sites = new ArrayList<>();\n        sites.add(new Site("今帰仁城跡", 2000, 2));\n        sites.add(new Site("首里城跡", 2000, 1));\n        String result = "";\n        for (int target = 1; target <= 2; target++) {\n            for (Site site : sites) {\n                if (site.year == 2000 && site.order == target) {\n                    result += site.name + " ";\n                }\n            }\n        }\n        System.out.println(result.trim());\n    }\n}', ['首里城跡 今帰仁城跡', '今帰仁城跡 首里城跡', '首里城跡', 'Error'], '首里城跡 今帰仁城跡', '外側の target が出力順を決めています。', 'target=1 で首里城跡、target=2 で今帰仁城跡が追加されます。', 'ネストしたループで順序を明示して抽出しています。', { '首里城跡 今帰仁城跡': '正解。外側のtargetが1,2の順に進むため、order=1の首里城跡が先、order=2の今帰仁城跡が後にresultへ追加されます。', '今帰仁城跡 首里城跡': 'targetが1から2へ進む順なので、order=1の首里城跡が先に追加され、この逆順にはなりません。', '首里城跡': 'target=2のループでもう一度sitesを調べるため、今帰仁城跡も結果に追加され1件だけでは終わりません。', 'Error': 'リストの追加・ネストしたループ・文字列連結はいずれも正常な操作で、例外は発生しません。' }),
      execute('final_execute_JP_java_02', 'collections', '石見銀山: final HashMap + method', 'HashMap の値とメソッド戻り値を使った出力を選んでください。', 'import java.util.HashMap;\npublic class Main {\n    static String label(String name, int year) {\n        return name + ":" + year;\n    }\n    public static void main(String[] args) {\n        HashMap<String, Integer> registered = new HashMap<>();\n        registered.put("石見銀山", 2007);\n        for (String name : registered.keySet()) {\n            System.out.println(label(name, registered.get(name)));\n        }\n    }\n}', ['石見銀山:2007', '2007:石見銀山', '石見銀山', 'Error'], '石見銀山:2007', 'keySet で名前を取り、get で年を取り出します。', 'name は石見銀山、registered.get(name) は2007なので、label が連結します。', 'HashMap のキーと値をメソッドへ渡す流れを追います。', { '石見銀山:2007': '正解。labelはname + ":" + yearの順で連結するため、名前の石見銀山が先、年の2007が後になります。', '2007:石見銀山': 'label(name, registered.get(name))はnameが先の引数に対応するため、年が先になることはありません。', '石見銀山': '戻り値には年も連結されているため、名前だけが出力されることはありません。', 'Error': 'keySetとgetの組み合わせは登録済みキーに対する正常な操作で、例外は発生しません。' }),
      execute('final_execute_JP_java_03', 'classes', '瀬戸内海国立公園: final objects + ArrayList', '国立公園オブジェクトを条件で選んだ出力を選んでください。', 'import java.util.ArrayList;\nclass Park {\n    String name; int year;\n    Park(String name, int year) { this.name = name; this.year = year; }\n    String label() { return name + ":" + year; }\n}\npublic class Main {\n    public static void main(String[] args) {\n        ArrayList<Park> parks = new ArrayList<>();\n        parks.add(new Park("瀬戸内海国立公園", 1934));\n        parks.add(new Park("別の公園", 1950));\n        for (Park park : parks) {\n            if (park.year <= 1934) {\n                System.out.println(park.label());\n            }\n        }\n    }\n}', ['瀬戸内海国立公園:1934', '別の公園:1950', '瀬戸内海国立公園\n別の公園', 'Error'], '瀬戸内海国立公園:1934', 'year <= 1934 を満たすオブジェクトだけです。', '1934年の瀬戸内海国立公園だけが条件を満たし、label() が呼ばれます。', 'オブジェクト、ArrayList、条件、メソッド出力を組み合わせています。', { '瀬戸内海国立公園:1934': '正解。park.year <= 1934を満たすのは瀬戸内海国立公園(1934)だけなので、label()が呼ばれてこの1件が出力されます。', '別の公園:1950': '別の公園のyearは1950で1934より大きいため、if条件を満たさず出力されません。', '瀬戸内海国立公園\n別の公園': '別の公園は条件を満たさないため出力されず、また出力形式はlabel()のname:year形式です。', 'Error': '条件分岐とメソッド呼び出しはいずれも正常な操作で、例外は発生しません。' }),
    ],
  },
  debug: {
    ...
    questions: [
      debug('final_debug_JP_java_01', 'exceptions', '別府温泉: final numeric parsing', '℃ を含む文字列から数値を取り出す処理のバグを修正します。', 'public class Main {\n    public static void main(String[] args) {\n        String temperature = "42℃";\n        int value = Integer.parseInt(temperature);\n        System.out.println(value + 1);\n    }\n}', [
        debugStep(1, '原因を特定', 'NumberFormatException が起きます。', '原因はどれですか。', ['42℃ をそのまま parseInt している', 'int は使えない', 'println がない'], '42℃ をそのまま parseInt している', '数字以外の文字が含まれています。', '℃ があるため整数として解析できません。', { '42℃ をそのまま parseInt している': '正解。"42℃"には数字以外の℃が含まれており、Integer.parseIntは数字だけの文字列を期待するためNumberFormatExceptionになります。', 'int は使えない': 'int型自体は正しく使え、valueの宣言にも問題はありません。', 'println がない': 'System.out.printlnは正しく書かれており、欠けているわけではありません。' }),
        debugStep(2, '修正を選ぶ', '安全に数値化する修正はどれですか。', '正しい修正はどれですか。', ['temperature.replace("℃", "") を parseInt する', 'temperature + 1 にする', 'parseInt を消す'], 'temperature.replace("℃", "") を parseInt する', '単位記号を取り除いてから解析します。', '数字部分だけにして parseInt します。', { 'temperature.replace("℃", "") を parseInt する': '正解。℃を取り除いて"42"だけの文字列にすることで、parseIntが正しく数値に変換できます。', 'temperature + 1 にする': 'これは文字列同士の連結になり"42℃1"という文字列になるだけで、数値変換の問題は解決しません。', 'parseInt を消す': 'parseIntを消すとvalueに文字列を代入できず、別のコンパイルエラーが発生します。' }),
        debugStep(3, '影響を説明', '修正後の動作を確認します。', '修正の影響はどれですか。', ['43 を出力できる', '42℃1 と出る', '常に0になる'], '43 を出力できる', '42 に 1 を足します。', '数値として加算できるため 43 になります。', { '43 を出力できる': '正解。℃を除いて変換したvalueは整数42となり、value + 1で43が正しく出力されます。', '42℃1 と出る': '修正後はvalueが整数として扱われるため、文字列のような連結結果にはなりません。', '常に0になる': 'replaceで℃を除いた"42"はparseIntで正しく42に変換され、0にはなりません。' }),
      ...
      debug('final_debug_JP_java_02', 'collections', '相撲の番付: final missing HashMap key', '番付 HashMap にないキーを読んで null 関連の失敗が起きるコードを修正します。', 'import java.util.HashMap;\npublic class Main {\n    public static void main(String[] args) {\n        HashMap<String, Integer> ranks = new HashMap<>();\n        ranks.put("横綱", 1);\n        ranks.put("大関", 2);\n        ranks.put("関脇", 3);\n        ranks.put("小結", 4);\n        ranks.put("前頭", 5);\n        int rank = ranks.get("幕下");\n        System.out.println(rank);\n    }\n}', [
        debugStep(1, '原因を特定', '実行時に null 関連の失敗が起きます。', '原因はどれですか。', ['HashMap にないキー 幕下 を get している', 'HashMap が空', '前頭の値が大きい'], 'HashMap にないキー 幕下 を get している', 'put されているキー一覧と get のキーを比べます。', '幕下 は登録されていないため null になります。', { 'HashMap にないキー 幕下 を get している': '正解。ranksには横綱〜前頭の5キーしか登録されておらず、幕下を読むとgetがnullを返し、int型への代入で失敗します。', 'HashMap が空': 'ranksには5つの番付が正しく登録されており、空ではありません。', '前頭の値が大きい': '前頭の値5は他の値と同程度の整数で、値の大きさが原因ではありません。' }),
        debugStep(2, '修正を選ぶ', '登録済みデータで前頭を読む修正はどれですか。', '正しい修正はどれですか。', ['ranks.get("前頭") にする', 'ranks.get(null) にする', 'ranks.clear() する'], 'ranks.get("前頭") にする', '問題で与えたキーの中から選びます。', '前頭 は put 済みなので値を取得できます。', { 'ranks.get("前頭") にする': '正解。前頭はputで登録済みのキーなので、getで値5を安全に取得できます。', 'ranks.get(null) にする': 'nullというキーは登録されていないため、これも同様にnullが返り問題は解決しません。', 'ranks.clear() する': 'clearはすべてのデータを削除してしまい、値を取得する目的とは逆の操作です。' }),
        debugStep(3, '影響を説明', '修正後の出力を確認します。', '修正の影響はどれですか。', ['5 を出力できる', 'null を出力する', 'キーが増える'], '5 を出力できる', '前頭に対応する値を見ます。', 'HashMap から登録済みキーの値を安全に取り出せます。', { '5 を出力できる': '正解。前頭に対応する値は5なので、ranks.get("前頭")は5を返し正しく出力されます。', 'null を出力する': '前頭は登録済みのキーなので、修正後はnullではなく値5が返ります。', 'キーが増える': '修正は読み取るキーを変えるだけで、HashMapに新たなキーが追加されるわけではありません。' }),
      ...
      debug('final_debug_JP_java_03', 'inheritance', '落語: final override signature', '落語の演目クラスで @Override が失敗するメソッド署名を修正します。', 'class Performance {\n    String describe(String style) {\n        return "performance:" + style;\n    }\n}\nclass Rakugo extends Performance {\n    @Override\n    String describe() {\n        return "座って演じる話芸";\n    }\n}\npublic class Main {\n    public static void main(String[] args) {\n        Performance p = new Rakugo();\n        System.out.println(p.describe("落語"));\n    }\n}', [
        debugStep(1, '原因を特定', '@Override のコンパイルエラーになります。', '原因はどれですか。', ['子クラスの describe に String 引数がない', 'return が文字列ではない', 'class が足りない'], '子クラスの describe に String 引数がない', '親メソッドの引数と子メソッドの引数を比べます。', '引数リストが違うため同じメソッドを上書きしていません。', { '子クラスの describe に String 引数がない': '正解。親のdescribe(String style)に対し子はdescribe()と引数が異なり、署名が一致しないため@Overrideがエラーになります。', 'return が文字列ではない': '"座って演じる話芸"は正しい文字列で、戻り値の型に問題はありません。', 'class が足りない': 'PerformanceとRakugoの両クラス定義は正しく書けており、不足はありません。' }),
        debugStep(2, '修正を選ぶ', '正しくオーバーライドする修正はどれですか。', '正しい修正はどれですか。', ['String describe(String style) にする', 'String describe(int style) にする', '@Override を消すだけ'], 'String describe(String style) にする', '親と同じ署名にします。', '引数と戻り値の互換性をそろえます。', { 'String describe(String style) にする': '正解。親のdescribe(String style)と同じ引数型・個数にすることで、正しくオーバーライドできます。', 'String describe(int style) にする': '引数の型がString styleではなくint styleになり、親と署名が一致せずオーバーライドになりません。', '@Override を消すだけ': '@Overrideを消すと単なる別メソッドになり、p.describe("落語")の呼び出し自体がコンパイルエラーのままです。' }),
        debugStep(3, '影響を説明', '修正後の呼び出しを確認します。', '修正の影響はどれですか。', ['Rakugo 側の describe が呼ばれる', '親の describe しか呼べない', '引数が不要になる'], 'Rakugo 側の describe が呼ばれる', '実体は Rakugo です。', '正しくオーバーライドされ、動的ディスパッチで子クラス側が選ばれます。', { 'Rakugo 側の describe が呼ばれる': '正解。pの実体はRakugoで、正しく署名を合わせたオーバーライドにより子クラス側のdescribeが呼ばれます。', '親の describe しか呼べない': '動的ディスパッチにより、実体であるRakugo側のオーバーライドされたメソッドが呼ばれます。', '引数が不要になる': '修正後もdescribeはString styleという引数を受け取り、呼び出し時に"落語"を渡す必要があります。' }),
