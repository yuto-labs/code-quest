...
export const C_REFERENCE_TOPICS = [
  topic('syntax-basics', 'C の基本構文ルール', 'comments', 'C は { } でブロックの範囲を示し、文の終わりには ; が必要です。インデント（字下げ）は見やすくするためのスタイルで、実行結果には影響しません。', [
    page('c-syntax-basics-braces-semicolons', '波かっこ { } とセミコロン', 'C では if や for、関数などの範囲を { から } で囲みます。1つの文の終わりには ; を付けます。', '#include <stdio.h>\n\nint main(void) {\n    int score = 80;\n    if (score >= 70) {\n        printf("pass\\n");\n    }\n    return 0;\n}', 'pass', ['L4: if の範囲を { で開始する。', 'L5: 文の終わりに ; を付ける。', 'L6: if の範囲を } で閉じる。'], [{ wrong: 'printf("pass\\n")', reason: '文の終わりに ; がないとコンパイルエラーになります。', correct: 'printf("pass\\n");' }, { wrong: 'if (score >= 70)\n    printf("pass\\n");\n}', reason: '{ で開けた範囲は同じ数の } で閉じる必要があります。', correct: 'if (score >= 70) {\n    printf("pass\\n");\n}' }], [{ prompt: '文の終わりに付ける記号は？', answer: ';' }, { prompt: '範囲を示す記号のペアは？', answer: '{ と }' }], 'インデント（字下げ）はコードを読みやすくするためのものです。波かっこの数が合っていれば、インデントの深さ自体は実行結果に影響しません。'),
    page('c-syntax-basics-case-comments', '大文字小文字とコメント', 'C は大文字小文字を区別します。コメントは // か /* */ を使います。', '#include <stdio.h>\n\nint main(void) {\n    // 1行コメント\n    int Score = 1;\n    /* 複数行\n       コメント */\n    printf("%d\\n", Score);\n    return 0;\n}', '1', ['L3: // から行末まではコメント（実行されない）。', 'L4: 変数名 Score は大文字始まり。score とは別の変数として区別される。', 'L5-L6: /* と */ で囲んだ複数行コメント。', 'L7: Score の値を出力する。'], [{ wrong: 'int score = 1;\nprintf("%d\\n", Score);', reason: 'score と Score は大文字小文字が違うため、別の名前として扱われコンパイルエラーになります。', correct: '変数名の大文字小文字を統一する（例: 両方 score にする）' }], [{ prompt: 'Score と score は同じ変数として扱われる？', answer: '扱われない（大文字小文字を区別する）' }, { prompt: '複数行コメントを開始する記号は？', answer: '/*' }]),
    reviewPage('c-syntax-basics', 'まとめ', '{ } の対応、; の有無、大文字小文字の違いは、C のコードを読むときに最初に確認したいポイントです。', [{ prompt: '範囲の開始と終了を示す記号は？', answer: '{ と }' }, { prompt: 'Score と score は別の変数？', answer: '別の変数（大文字小文字を区別する）' }]),
