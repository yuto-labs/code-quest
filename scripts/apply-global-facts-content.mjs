...
import { NEW_GLOBAL_FACTS } from './new-global-facts-content.mjs';

const FILE = 'src/data/sql/global_facts.js';

function findBlock(text, factId) {
  const idMarker = `"globalFactId": "${factId}",`;
...
for (const [factId, content] of Object.entries(NEW_GLOBAL_FACTS)) {
...
console.log(`Applied ${applied}/${Object.keys(NEW_GLOBAL_FACTS).length}`);
